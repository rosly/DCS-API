...
if http_wrapper == nil then _G.http_wrapper = {} end

-- http_wrapper.a_do_scriturn() requires io.open() and lfs.tempdir(). however, these
...
-- end of this file, but are captured by http_wrapper.a_do_scriturn(), because Lua
...
function http_wrapper.a_do_scriturn(func, maxdepth)
    local file, err = _open(_temp, "w")
    if file == nil then
        log.write("MissionScripting.lua", log.ERROR, string.format("ATCServer:http_wrapper.a_do_scriturn: error opening %q: %s", _temp, err))
        return
    end
    ...
    if success then
        ...
        file:write('{"ok":false,"function":"http_wrapper.a_do_scriturn","error":"%s"}', tostring(result_or_errmsg))
    end
    file:close()
end
